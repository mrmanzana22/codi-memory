...
class TestEmptyDbBaseline:
    ...
    def test_fts_baseline_creates_tables(self, tmp_path):
        ...
        # 022, 032 are postgres-only and should be skipped
        assert "022" in result["skipped"], "Migration 022 (postgres-only) should be skipped"
        assert "032" in result["skipped"], "Migration 032 (postgres-only) should be skipped"
        assert result["current_version"] == "033"
...
class TestNoCreateTableOutsideMigrations:
    ...
    def test_no_create_table_outside_migrations(self):
        ...
        WHITELIST = {
            ...
            "ollama_router.py",
            "cx_observability.py",
            "source_tracking.py",  # PG-only table, not SQLite
            "recall_eval.py",       # tables also in migration 029
        }

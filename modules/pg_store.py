...
class PGMemoryStore:
    ...
    def search(
        ...
        # Query expansion: bilingual aliases + acronyms (replaces inline _FTS_ALIASES)
        try:
            from modules.query_expansion import build_expanded_tsquery
            fts_query = build_expanded_tsquery(query)
        except Exception:
            # Fallback: basic term joining
            _ES_STOPS = frozenset({
    ...
    def search_fts(self, query: str, limit: int = 20) -> list[dict]:
        ...
        try:
            from modules.query_expansion import build_expanded_tsquery
            tsquery = build_expanded_tsquery(query)
        except Exception:
            # Fallback: basic term joining
            _ES_STOPS = frozenset({
                "que", "de", "en", "el", "la", "los", "las", "un", "una",
